(function attachSidepanelHotmailManager(globalScope) {
  function createHotmailManager(context = {}) {
    ...
    function buildAccountActionKey(accountId, action) {
      return `${String(accountId || '').trim()}::${String(action || '').trim()}`;
    }
    ...
    function renderHotmailAccounts() {
      ...
      dom.hotmailAccountsList.innerHTML = visibleAccounts.map((account) => {
        const verifyBusy = accountActionInFlight.has(buildAccountActionKey(account.id, 'verify'));
        return `
        <div class="hotmail-account-item${account.id === currentId ? ' is-current' : ''}">
          ...
          <div class="hotmail-account-actions">
            ...
            <button class="btn btn-primary btn-sm" type="button" data-account-action="verify" data-account-id="${helpers.escapeHtml(account.id)}"${verifyBusy ? ' disabled' : ''}>${verifyBusy ? '校验中...' : '校验'}</button>
    ...
    async function handleAccountListClick(event) {
      ...
      const allowWhileOtherActionRuns = action === 'verify';
      if (actionInFlight && !allowWhileOtherActionRuns) {
        return;
      }

      const actionKey = buildAccountActionKey(accountId, action);
      if (accountActionInFlight.has(actionKey)) {
        return;
      }

      if (!allowWhileOtherActionRuns) {
        actionInFlight = true;
      }
      accountActionInFlight.add(actionKey);
      ...
      try {
        ...
        accountActionInFlight.delete(actionKey);
