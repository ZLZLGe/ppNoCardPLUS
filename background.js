...
const IP_PROXY_BYPASS_LIST = ['<local>', 'localhost', '127.0.0.1', '149.88.88.6'];
...
const HOTMAIL_REMOTE_MAIL_API_ENDPOINTS = Object.freeze(['mail_new', 'mail_all']);
const DEFAULT_HOTMAIL_REMOTE_BASE_URL = 'http://149.88.88.6:3000';
...
function normalizeHotmailRemoteBaseUrl(rawValue = '') {
  ...
  try {
    ...
    if (
      parsed.pathname.endsWith('/api/mail-new')
      || parsed.pathname.endsWith('/api/mail-all')
      || parsed.pathname.endsWith('/api/mail_new')
      || parsed.pathname.endsWith('/api/mail_all')
      || parsed.pathname === '/api.html'
    ) {
      parsed.pathname = '';
      parsed.search = '';
      parsed.hash = '';
...
function normalizeHotmailMailboxLabel(mailbox = 'INBOX') {
  return /^junk(?:\s*e-?mail|\s*email)?$/i.test(String(mailbox || '').trim()) ? 'Junk' : 'INBOX';
}

function buildHotmailRemoteEndpoint(baseUrl, endpointName, account, mailbox = 'INBOX') {
  const normalizedBaseUrl = normalizeHotmailRemoteBaseUrl(baseUrl);
  if (!normalizedBaseUrl) {
    throw new Error('Hotmail API 对接地址未填写，请填写例如 http://149.88.88.6:3000');
  }

  const url = new URL(`/api/${endpointName}`, `${normalizedBaseUrl}/`);
  url.searchParams.set('refresh_token', String(account?.refreshToken || ''));
  url.searchParams.set('client_id', String(account?.clientId || ''));
  url.searchParams.set('email', String(account?.email || ''));
  url.searchParams.set('mailbox', normalizeHotmailMailboxLabel(mailbox));
  url.searchParams.set('response_type', 'json');
  return url.toString();
}

function firstHotmailRemoteText(...values) {
...
function parseHotmailRemoteTimestamp(rawValue) {
  const text = String(rawValue || '').trim();
  if (!text) {
    return { timestampMs: 0, iso: '' };
  }
  if (/^\d+$/.test(text)) {
    const number = Number(text);
    if (Number.isFinite(number) && number > 0) {
      const timestampMs = number > 10_000_000_000 ? number : number * 1000;
      return {
        timestampMs,
  ...
  return { timestampMs: 0, iso: text };
}

function normalizeHotmailRemoteItems(payload) {
  ...
  const directKeys = new Set(['text', 'date', 'html', 'subject', 'otp', 'code', 'body', 'content', 'message']);
  for (const key of ['data', 'items', 'mails', 'messages', 'list', 'rows']) {
    const value = payload[key];
    if (Array.isArray(value)) {
      return value.filter((item) => item && typeof item === 'object');
    }
    if (value && typeof value === 'object' && [...directKeys].some((item) => item in value)) {
      return [value];
    }
  }

  return [...directKeys].some((item) => item in payload) ? [payload] : [];
}

function normalizeHotmailRemoteMessage(rawMessage, mailbox, endpointName, index) {
  const source = rawMessage && typeof rawMessage === 'object' ? rawMessage : {};
  const senderRaw = firstHotmailRemoteText(source.send, source.sender, source.from, source.from_email, source.sender_email);
  const senderMatch = senderRaw.match(/<([^<>@\s]+@[^<>\s]+)>/);
  const senderAddress = senderMatch?.[1] || senderRaw;
  const body = firstHotmailRemoteText(
    source.text,
    source.bodyPreview,
    source.preview,
    source.html,
  ...
  const dateRaw = firstHotmailRemoteText(
    source.date,
    source.receivedAt,
    source.received_at,
    source.createdAt,
  ...
  const parsedDate = parseHotmailRemoteTimestamp(dateRaw);
  const logicalMailbox = normalizeHotmailMailboxLabel(mailbox);
  const messageId = firstHotmailRemoteText(source.id, source.messageId, source.internetMessageId)
    || `gpt-mail-api:${endpointName}:${logicalMailbox}:${index}:${parsedDate.timestampMs || dateRaw}`;

  return {
    id: messageId,
    mailbox: logicalMailbox,
    subject: firstHotmailRemoteText(source.subject, source.title),
    from: {
      emailAddress: {
    ...
    receivedDateTime: parsedDate.iso,
    receivedTimestamp: parsedDate.timestampMs,
  };
}

async function fetchHotmailRemoteEndpointPayload(url, timeoutMs, label) {
...
async function requestHotmailRemoteMailbox(account, mailbox = 'INBOX') {
  ...
  try {
    let lastError = null;
    for (const endpointName of HOTMAIL_REMOTE_MAIL_API_ENDPOINTS) {
      const label = `${endpointName} ${logicalMailbox}`;
      const { response, payload, text } = await fetchHotmailRemoteEndpointPayload(
        buildHotmailRemoteEndpoint(serviceSettings.remoteBaseUrl, endpointName, account, logicalMailbox),
        timeoutMs,
        label
      );
      ...
      if (!response.ok || payload?.ok === false) {
        ...
        lastError = new Error(`${label}: ${errorText}`);
        continue;
      }

      const rawItems = normalizeHotmailRemoteItems(payload);
      const messages = rawItems
        .slice(0, 10)
        .map((item, index) => normalizeHotmailRemoteMessage(item, logicalMailbox, endpointName, index));
      if (messages.length || endpointName === HOTMAIL_REMOTE_MAIL_API_ENDPOINTS[HOTMAIL_REMOTE_MAIL_API_ENDPOINTS.length - 1]) {
        return {
          mailbox: logicalMailbox,
          payload: {
            source: 'gpt-mail-api',
            transport: 'gpt-mail-otp',
    ...
    throw lastError || new Error(`${logicalMailbox}: 未返回邮件数据`);
  } catch (err) {
    if (err?.name === 'AbortError' || /\btimeout\b/i.test(String(err?.message || ''))) {
      throw new Error(`Hotmail API 对接请求超时（>${Math.round(timeoutMs / 1000)} 秒）：${logicalMailbox}`);
    }
    throw new Error(`Hotmail API 对接请求失败：${err.message}`);
  }
}

async function quickVerifyHotmailRemoteMailbox(account) {
  if (!account?.email) {
    throw new Error('Hotmail 账号缺少邮箱地址。');
  }
  if (!account?.clientId) {
  ...
  const serviceSettings = getHotmailServiceSettings(await getState());
  const mailbox = 'INBOX';
  const endpointName = 'mail_new';
  const label = `${endpointName} ${mailbox}`;

  try {
    const { response, payload, text } = await fetchHotmailRemoteEndpointPayload(
      buildHotmailRemoteEndpoint(serviceSettings.remoteBaseUrl, endpointName, account, mailbox),
      timeoutMs,
      label
    );
    if (!response.ok || payload?.ok === false) {
    ...
    const messages = normalizeHotmailRemoteItems(payload);
    return {
      mailbox,
    ...
    if (err?.name === 'AbortError' || /\btimeout\b/i.test(String(err?.message || ''))) {
      throw new Error(`Hotmail API 快速校验超时（>${Math.round(timeoutMs / 1000)} 秒）：${mailbox}`);
    }
    throw new Error(`Hotmail API 快速校验失败：${err.message}`);
  }
}
...
async function quickVerifyHotmailAccount(accountId) {
  const state = await getState();
  const account = findHotmailAccount(state.hotmailAccounts, accountId);
  if (!account) {
    throw new Error('未找到需要校验的 Hotmail 账号。');
  ...
  if (serviceSettings.mode === HOTMAIL_SERVICE_MODE_REMOTE) {
    verifyResult = await quickVerifyHotmailRemoteMailbox(account);
  } else {
    const result = await requestHotmailLocalMessages(account, ['INBOX']);
    verifyResult = {
      mailbox: 'INBOX',
      messageCount: result.mailboxResults[0]?.count || 0,
  ...
  const savedAccount = await upsertHotmailAccount(applyHotmailApiResultToAccount(account, {
    nextRefreshToken: '',
  }));
  return {
    account: savedAccount,
    messageCount: Number(verifyResult?.messageCount || 0),
    mailbox: verifyResult?.mailbox || 'INBOX',
    transport: verifyResult?.transport || '',
...
async function waitForTabUrlMatchUntilStopped(tabId, matcher, options = {}) {
  ...
  while (true) {
    ...
    if (Date.now() - startTime > 60000) {
      await addLog(`检测到标签页 ${tabId} 等待跳转卡住超过 60 秒，正在刷新页面...`, 'warn');
...
async function waitForTabCompleteUntilStopped(tabId, options = {}) {
  ...
  while (true) {
    ...
    if (Date.now() - startTime > 60000) {
      await addLog(`检测到标签页 ${tabId} 加载卡住超过 60 秒，正在刷新页面...`, 'warn');
      await chrome.tabs.reload(tabId).catch(() => null);
      startTime = Date.now(); // 重置计时器
    }
    await sleepWithStop(retryDelayMs);
...
async function ensureContentScriptReadyOnTabUntilStopped(source, tabId, options = {}) {
  ...
  while (true) {
    ...
    if (Date.now() - startTime > 60000) {
      await addLog(`检测到标签页 ${tabId} 内容脚本 ${source} 卡住未就绪超过 60 秒，正在刷新页面...`, 'warn');
      await chrome.tabs.reload(tabId).catch(() => null);
      startTime = Date.now(); // 重置计时器
    }
...
async function sendTabMessageUntilStopped(tabId, source, message, options = {}) {
  ...
  while (true) {
    ...
    try {
      ...
      if (Date.now() - startTime > 60000) {
        await addLog(`检测到向标签页 ${tabId} 发送消息卡住超过 60 秒，正在刷新页面...`, 'warn');
        await chrome.tabs.reload(tabId).catch(() => null);
        startTime = Date.now(); // 重置计时器
      }
      await sleepWithStop(retryDelayMs);
...
async function setNodeStatus(nodeId, status) {
  ...
  if (status === 'running') {
    ...
    if (currentIndex >= 0) {
      for (let index = currentIndex + 1; index < activeNodeIds.length; index += 1) {
        ...
        downstreamChangedNodes.push(downstreamNodeId);
      }
    }
  }
  ...
  for (const downstreamNodeId of downstreamChangedNodes) {
    chrome.runtime.sendMessage({
      type: 'NODE_STATUS_CHANGED',
      payload: { nodeId: downstreamNodeId, status: 'pending' },
    }).catch(() => { });
  }
  chrome.runtime.sendMessage({
    type: 'NODE_STATUS_CHANGED',
