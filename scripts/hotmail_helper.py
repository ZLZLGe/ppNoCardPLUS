...
GPT_MAIL_API_BASE = "http://149.88.88.6:3000"
GPT_MAIL_API_TIMEOUT_SECONDS = 8
GPT_MAIL_API_ENDPOINTS = ("mail_new", "mail_all")
...
def parse_gpt_mail_date(raw_value):
    ...
    try:
        if text.isdigit():
            number = int(text)
            timestamp_ms = number if number > 10_000_000_000 else number * 1000
            return timestamp_ms, to_iso_string(timestamp_ms)
        parsed = datetime.fromisoformat(text.replace("Z", "+00:00"))
        if parsed.tzinfo is None:
            parsed = parsed.replace(tzinfo=timezone.utc)
        timestamp_ms = int(parsed.timestamp() * 1000)
        return timestamp_ms, parsed.astimezone(timezone.utc).isoformat().replace("+00:00", "Z")
    except Exception:
        return 0, text
...
def normalize_gpt_mail_items(payload):
    if isinstance(payload, list):
        return [item for item in payload if isinstance(item, dict)]
    if not isinstance(payload, dict):
        return []
    direct_keys = {"text", "date", "html", "subject", "otp", "code", "body", "content", "message"}
    for key in ("data", "items", "mails", "messages", "list", "rows"):
        value = payload.get(key)
        if isinstance(value, list):
            return [item for item in value if isinstance(item, dict)]
        if isinstance(value, dict) and direct_keys.intersection(value.keys()):
            return [value]
...
def build_gpt_mail_url(endpoint_name, refresh_token, client_id, email_addr, mailbox):
    query = urlencode(
        {
            "refresh_token": refresh_token,
            "client_id": client_id,
    ...
    return f"{GPT_MAIL_API_BASE.rstrip('/')}/api/{endpoint_name}?{query}"


def fetch_gpt_mail_payload(endpoint_name, refresh_token, client_id, email_addr, mailbox):
    url = build_gpt_mail_url(endpoint_name, refresh_token, client_id, email_addr, mailbox)
    request = Request(url=url, method="GET", headers={"Accept": "application/json"})
    started_at = time.monotonic()
    try:
        with urlopen(request, timeout=GPT_MAIL_API_TIMEOUT_SECONDS) as response:
        ...
        return {
            ...
            "mailbox": normalize_mailbox_label(mailbox),
            "url": url,
            "error": f"HTTPError {exc.code}: {compact_text(body or exc)}",
            "elapsed_ms": int((time.monotonic() - started_at) * 1000),
        }
    except Exception as exc:
        return {
...
def normalize_gpt_mail_message(mail, mailbox, endpoint_name, index):
    sender_raw = first_non_empty(mail.get("send"), mail.get("sender"), mail.get("from"), mail.get("from_email"))
    sender_name, sender_addr = parseaddr(sender_raw)
    if not sender_addr and sender_raw:
        sender_addr = sender_raw
    subject = first_non_empty(mail.get("subject"), mail.get("title"))
    ...
    date_raw = first_non_empty(mail.get("date"), mail.get("receivedAt"), mail.get("received_at"), mail.get("createdAt"), mail.get("created_at"), mail.get("time"))
    timestamp_ms, received_iso = parse_gpt_mail_date(date_raw)
    message_id = first_non_empty(mail.get("id"), mail.get("messageId"), mail.get("internetMessageId"))
    if not message_id:
        message_id = f"gpt-mail-otp:{endpoint_name}:{mailbox}:{index}:{timestamp_ms or date_raw}"
    return {
        "id": message_id,
        "mailbox": normalize_mailbox_label(mailbox),
        "subject": subject,
...
def collect_gpt_mail_messages(email_addr, client_id, refresh_token, mailboxes, top):
    mailbox_results = []
    all_messages = []
    traces = []
    had_success = False
    ...
    for mailbox in mailboxes or ["INBOX"]:
        ...
        selected_messages = []
        for endpoint_name in GPT_MAIL_API_ENDPOINTS:
            trace = fetch_gpt_mail_payload(endpoint_name, refresh_token, client_id, email_addr, mailbox)
            traces.append(trace)
            if not trace.get("ok"):
                log_info(
                    "gpt mail fetch failed "
                    f"endpoint={endpoint_name} mailbox={logical_mailbox} "
                    f"elapsedMs={trace.get('elapsed_ms', 0)} detail={trace.get('error', '')}"
                )
                continue
            had_success = True
            items = normalize_gpt_mail_items(trace.get("payload"))
            selected_messages = [
                normalize_gpt_mail_message(item, logical_mailbox, endpoint_name, index)
                for index, item in enumerate(items[: max(1, min(int(top or FETCH_LIMIT_DEFAULT), 30))])
            ]
            log_info(
                "gpt mail fetch success "
                f"endpoint={endpoint_name} mailbox={logical_mailbox} "
                f"count={len(selected_messages)} elapsedMs={trace.get('elapsed_ms', 0)}"
            )
            if selected_messages or endpoint_name == GPT_MAIL_API_ENDPOINTS[-1]:
                break
        ...
        all_messages.extend(selected_messages)

    if not had_success and traces:
        detail = " | ".join(
            f"{item.get('endpoint')}:{item.get('mailbox')}:{item.get('error')}"
            for item in traces
            if not item.get("ok")
        )
        raise RuntimeError(f"gpt mail otp fetch failed on all endpoints: {compact_text(detail, 1200)}")
    ...
    def do_POST(self):
        try:
            ...
            if request_path == "/messages":
                result = collect_gpt_mail_messages(email_addr, client_id, refresh_token, mailboxes, top)
                json_response(self, 200, {
                    "ok": True,
                    "messages": result["messages"],
                    "mailboxResults": result["mailboxResults"],
                    "nextRefreshToken": "",
                    "tokenEndpoint": "gpt-mail-otp",
