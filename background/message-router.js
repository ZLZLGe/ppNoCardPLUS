        ...
        case 'AUTHORIZE_HOTMAIL_ACCOUNT': {
          ...
          try {
            const result = typeof quickVerifyHotmailAccount === 'function'
              ? await quickVerifyHotmailAccount(accountId)
              : await verifyHotmailAccount(accountId);
            Promise.resolve(setCurrentHotmailAccount(result.account.id, { markUsed: false, syncEmail: true }))
              .catch((error) => console.warn('[MessageRouter] Hotmail current account sync failed:', error));
            Promise.resolve(addLog(`Hotmail 账号 ${result.account.email} 校验通过，可直接用于收信。`, 'ok'))
              .catch((error) => console.warn('[MessageRouter] Hotmail verify log failed:', error));
